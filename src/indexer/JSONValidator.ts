...
const JSON_VALIDATOR_ERROR = 'JSON_VALIDATOR_ERROR';
...
class JSONValidator {
  ...
  public validate(jsonObject): AppError {
    ...
    const propertiesValuesError = JSONValidator.checkPropertiesValues(jsonObject);
  ...
  private checkPropertiesNames(jsonObjectKeys: string[]): AppError {
    for (const key of jsonObjectKeys) {
      if (this.validJsonProps.indexOf(key) < 0) {
        ...
        return new AppError(JSON_VALIDATOR_ERROR, ErrorCodes.UNKNOWN_PROPERTY, desc);
      }
    }
    return new NullError();
  }
  ...
  private static checkPropertiesValues(jsonObject): AppError {
    ...
    for (const key of keys) {
      ...
      if (!value || value?.length === 0) {
        ...
        return new AppError(JSON_VALIDATOR_ERROR, ErrorCodes.EMPTY_PROPERTY_VALUE, desc);
      }
    }
    return new NullError();
  }
  ...
  private checkPropertiesNumber(jsonObjectKeys: string[]): AppError {
    ...
    if (propsNumber < this.obligatoryJsonProps.length) {
      ...
      return new AppError(JSON_VALIDATOR_ERROR, ErrorCodes.INSUFFICIENT_PROPERTIES_NUMBER, desc);
    }
    if (propsNumber > this.propsLimit) {
      ...
      return new AppError(JSON_VALIDATOR_ERROR, ErrorCodes.PROPERTIES_NUMBER_EXCEEDED, desc);
    }
    return new NullError();
  }
